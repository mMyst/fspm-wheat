...
def main(simulation_length, forced_start_time=0, run_simu=True, run_postprocessing=True, generate_graphs=True, run_from_outputs=False, stored_times=None,
    ...
    COEF_DELTA_TIL = 0.1 #delta til = coef * plastochrone : time buffer for tiller emission 
    ...
    management = pd.read_csv(os.path.join(INPUTS_DIRPATH, MANAGEMENT_FILENAME), header=None, index_col=0).squeeze("columns")
                ...
                for t_senescwheat in range(t_caribu, t_caribu + SENESCWHEAT_TIMESTEP, SENESCWHEAT_TIMESTEP):
                    ...
                    for t_farquharwheat in range(t_senescwheat, t_senescwheat + SENESCWHEAT_TIMESTEP, FARQUHARWHEAT_TIMESTEP):
                        ...
                        for t_elongwheat in range(t_farquharwheat, t_farquharwheat + FARQUHARWHEAT_TIMESTEP, ELONGWHEAT_TIMESTEP):
                            ...
                            elongwheat_facade_.run(Tair, Tsoil, tillers_replications, Zsowing, optimal_growth_option=False, option_static=option_static)
                            ...
                            #TODO replace with a function from elongwheat_tillering 
                            ms_vid = next(vid for vid, label in g.property('label').items() if label == 'MS') #get MS vid id 
                            if g.property('teq_since_primordium')[ms_vid] < elongwheat_parameters.PARAMETERS.PLASTOCHRONE*COEF_DELTA_TIL:
                                GAIp = elongwheat_tillering.compute_1plant_GAIp(adel_wheat.scene(g),PLANT_DENSITY[1])                                
                                initiated_leaves = g.property('nb_leaves')[ms_vid]
                                tillers_replications = elongwheat_tillering.tiller_initiation(tillers_replications, initiated_leaves, GAIp,GAIc=0.1) 
                                #0.1 GAIc allows for 4 tillers to emerge with 250 plants/m2
...
if __name__ == '__main__':
    main(4000, forced_start_time=800, run_simu=True, run_postprocessing=True, generate_graphs=True, run_from_outputs=False,
         ...
         heterogeneous_canopy=True, external_soil_model=True, N_fertilizations={2949: 357143, 4029: 1000000},
