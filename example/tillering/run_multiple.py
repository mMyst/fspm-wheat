...
def run_single_simulation(params):
    ...
    # Création de noms de dossiers uniques basés sur tous les paramètres explorés
    folder_suffix = f"GAIc_{gaic}_dens_{density}_delay_{coef_delay}_buf_{coef_buffer}"
    
    # Ajout de la couche "parallel_tests" dans les chemins
    output_path = os.path.join(base_dir, f"outputs_{folder_suffix}")
    post_path = os.path.join(base_dir, f"postprocessing_{folder_suffix}")
    graph_path = os.path.join(base_dir, f"graphs_{folder_suffix}")

    # Création des répertoires (exist_ok=True prévient les erreurs si le dossier parent a déjà été créé par un autre processus)
    ...
    try:
        # Appel du main avec les paramètres spécifiques
        fspm_main.main(
